...
//        userService.getUser(userId);
        List<Request> requests = requestRepository.findAllByEventId_Id(eventId);
...
//        Event event = eventPublicService.getEventById(eventId);
    ...
    public Event updateEvent(Event event, UpdateEventRequest updateEventRequest) {
        ...
        if (updateEventRequest.getCategory() != null) event.setCategory(categoryRepository.findById(updateEventRequest.getCategory())
                .orElseThrow(() -> new NotFoundException("Category is not found!")));
