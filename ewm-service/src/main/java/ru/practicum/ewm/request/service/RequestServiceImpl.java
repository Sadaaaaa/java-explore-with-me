...
public class RequestServiceImpl implements RequestService {
    ...
    public ParticipationRequestDto createUserRequest(int userId, int eventId) {
        ...
        if (!event.getRequestModeration()) {
            ...
            // увеличиваем счетчик подтвержденных реквестов в ивенте
    ...
    public ParticipationRequestDto cancelUserRequest(int userId, int requestId) {
        ...
        Event event = requestForCancel.getEventId();

        // уменьшаем счетчик подтвержденных реквестов в ивенте
        if (requestForCancel.getStatus() == RequestStatus.CONFIRMED) {
            event.setConfirmedRequests(event.getConfirmedRequests() - 1);
            eventRepository.save(event);
        }
